...
export default function RuHomePage() {
  ...
  return (
    <main>
      ...
      <section className="relative overflow-hidden border-t border-black/5 bg-[#1e252b] py-18 text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(252,163,17,0.16),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(255,255,255,0.05),transparent_24%)]" aria-hidden="true" /><div className="relative mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-2 lg:px-10"><div><div className="text-xs uppercase tracking-[0.3em] text-[#d5bd9e]">{copy.home.marketEyebrow}</div><h2 className="mt-3 text-3xl font-semibold tracking-tight md:text-5xl">{copy.home.marketTitle}</h2></div></div></section>
      <section className="ui-section ui-section-cool"><div className="max-w-3xl"><div className="text-xs uppercase tracking-[0.3em] text-[#8f7d67]">{copy.home.faqEyebrow}</div><h2 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl md:text-5xl">{copy.home.faqTitle}</h2></div><div className="mt-8 grid gap-4 md:mt-10 md:gap-5">{faqs.map((item) => <div key={item.q} className="ui-card px-5 py-5 sm:px-6 sm:py-6 md:px-7 md:py-7"><h3 className="text-lg font-semibold tracking-tight">{item.q}</h3><p className="mt-3 text-sm leading-7 text-[#59646e] md:text-base">{item.a}</p></div>)}</div></section>
    </main>
  );
}
